...
describe("a pill", () => {
  ...
  it("positions one highlight over the live option", () => {
    ...
    const group = container.querySelector(".sg-pill") as HTMLElement;
    expect(group.style.getPropertyValue("--i")).toBe("1");
    expect(group.style.getPropertyValue("--n")).toBe("2");
    expect(container.querySelectorAll(".sg-pill__lit")).toHaveLength(1);
  });
  ...
  it("lights the first option rather than none when the value is a stranger", () => {
    ...
    expect((container.querySelector(".sg-pill") as HTMLElement).style.getPropertyValue("--i")).toBe("0");
  });
});
