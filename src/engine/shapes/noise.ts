 ...
 * implementation, with the permutation table shuffled by a seeded PRNG so the
 * same seed always gives the same blob.
 */

/** mulberry32: tiny, fast, good enough to shuffle a table. */
...
/** Returns a noise function in -1..1 for any point in space. */
export function simplex3(seed: number): (x: number, y: number, z: number) => number {
  const rng = mulberry32(Math.floor(seed * 1000003) + 1);
...
/** A seeded 0..1 random, for the shapes that jitter points rather than sample a field. */
